...
static void capture_model(CPUState*cpu,uint32_t address) {
    if(!ridge_scene_state(psx_mod_read_half(RR_STATE)))return;
    ...
    if((!car&&!scenery&&!effects)||(car&&!owner)||cpu->gpr[6]>=319)return;
    static int logged;
    ...
    if(scenery){ridge_scenery_compare(m);if(ridge_scenery_owns(site))ridge_model_count--;}
}
