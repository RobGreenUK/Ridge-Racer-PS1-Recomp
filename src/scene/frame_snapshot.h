#pragma once
// Private same-host RRSCENE6 marker recording, readable by the normal preview.
...
inline void saveFrameSnapshot(const std::string&path,const ridge::Frame&f){
    ...
    out.write("RRSCENE6",8);word(3);word(2);
    ...
    for(int sample=0;sample<2;sample++){
        ...
        word(f.hud.size());for(uint32_t v:f.hud)word(v);word(f.menuBackCount);
    }
}
